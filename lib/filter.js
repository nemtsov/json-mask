...
function _funcMask (obj, item, tree) {
  return tree.reduce((obj, key) => obj[key], obj).apply(obj)
}
      ...
      if (value.type === 'function') {
        if (key.indexOf('.') !== -1) {
          ...
          if (Array.isArray(obj[key])) {
            ret = obj[key].map(obj => _funcMask(obj, tree.slice(1)))
          } else {
            ret = _funcMask(obj[key], tree.slice(1))
          }
        } else {
          if (Array.isArray(obj[key])) {
            ret = obj[key].map(func => func.apply(obj))
          } else {
            ret = obj[key].apply(obj)
          }
        }
      } else {
